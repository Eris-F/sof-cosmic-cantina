...
import { drawCatShip, drawSmiski, drawJellycat, drawTieFighter, drawBreadBullet } from '../sprites';

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

type TutorialPageDrawFn = (ctx: CanvasRenderingContext2D, time: number) => void;

interface TutorialPage {
  readonly title: string;
  readonly draw: TutorialPageDrawFn;
}
...
/**
 * Tutorial page definitions — data + drawing callbacks.
 * Each page.draw receives (ctx, time) and draws pixel art illustrations.
 */
const PAGES: readonly TutorialPage[] = [
  {
    ...
    draw: (ctx: CanvasRenderingContext2D, time: number): void => {
      ...
      const fingerY = getGameHeight() * 0.82 + Math.cos(time * 1.5) * 20;
  ...
  },
  {
    ...
    draw: (ctx: CanvasRenderingContext2D, _time: number): void => {
      ...
      const weapons: readonly { readonly name: string; readonly description: string; readonly color: string }[] = [
        { name: 'STANDARD', description: 'Single shot', color: '#fff' },
        { name: 'SHOTGUN', description: '5-shot spread', color: '#ff8844' },
        { name: 'SNIPER', description: 'Piercing shots', color: '#44ccff' },
        { name: 'MINIGUN', description: 'Rapid tiny shots', color: '#44ff44' },
      ];
      ...
      for (let i = 0; i < weapons.length; i++) {
        ...
        ctx.fillText(w.name, CANVAS_WIDTH / 2 - 30, y);
        ctx.fillStyle = '#666';
        ctx.font = '9px monospace';
        ctx.fillText(w.description, CANVAS_WIDTH / 2 + 60, y);
      }
      ctx.textAlign = 'left';
    },
  },
  {
    title: 'POWER-UPS',
    draw: (ctx: CanvasRenderingContext2D, _time: number): void => {
      const items: readonly { readonly letter: string; readonly color: string; readonly label: string }[] = [
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        ctx.font = 'bold 10px monospace';
        ctx.textAlign = 'center';
        ctx.fillText(it.letter, CANVAS_WIDTH / 2 - 82, y + 3);
        ctx.fillStyle = '#ccc';
        ctx.font = '9px monospace';
        ctx.textAlign = 'left';
        ctx.fillText(it.label, CANVAS_WIDTH / 2 - 65, y + 3);
      }
      ctx.textAlign = 'center';
      ctx.fillStyle = '#666';
      ctx.font = '9px monospace';
  ...
  },
  {
    ...
    draw: (ctx: CanvasRenderingContext2D, time: number): void => {
      ...
      const mods: readonly { readonly name: string; readonly color: string; readonly desc: string }[] = [
      ...
      for (let i = 0; i < mods.length; i++) {
        ...
        ctx.fillText(mod.name, CANVAS_WIDTH / 2 - 40, y);
        ctx.globalAlpha = 1;
        ctx.fillStyle = '#888';
        ctx.font = '8px monospace';
        ctx.fillText(mod.desc, CANVAS_WIDTH / 2 + 55, y);
      }
...
export const TUTORIAL_PAGE_COUNT: number = PAGES.length;

/**
 * Renders the tutorial screen. Pure read-only — draws pixels, returns nothing.
 */
