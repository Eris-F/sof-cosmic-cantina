...
const COMBO_LABELS: readonly string[] = ['', '', 'DOUBLE!', 'TRIPLE!', 'QUAD!', 'PENTA!', 'MEGA!', 'ULTRA!', 'INSANE!'];
...
let streakAnnouncement: StreakAnnouncement | null = null;

export function registerKill(x: number, y: number): void {
...
export function updateCombo(dt: number): void {
  if (comboTimer > 0) {
    comboTimer -= dt;
    if (comboTimer <= 0) {
      comboCount = 0;
...
export function renderCombo(ctx: CanvasRenderingContext2D): void {
...
export function renderStreakAnnouncement(ctx: CanvasRenderingContext2D): void {
