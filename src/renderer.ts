...
import { submitHighScore, startNewGame } from './game';
import type { Game } from './game';
import { shareScore } from './share';
import { drawCatLife, drawCatShip, drawSmiski, drawJellycat, drawTieFighter } from './sprites';
import { DIFFICULTIES, DIFFICULTY_KEYS } from './difficulty';
import { sfxMenuSelect } from './audio';
import { saveWallet } from './shop';
import { handleShopTouch } from './shopui';
import { swapWeapon } from './weapons';
import { SKILL_BRANCHES, upgradeSkill, loadSkillLevels } from './skilltree';
import { getTutorialPageCount } from './tutorial';
...
function renderMenu(ctx: CanvasRenderingContext2D, game: Game): void {
  const t = game.time;
...
function renderHUD(ctx: CanvasRenderingContext2D, game: Game): void {
...
function renderWaveText(ctx: CanvasRenderingContext2D, game: Game): void {
...
function renderPaused(ctx: CanvasRenderingContext2D, game: Game): void {
  // Dim overlay
...
function renderGameOver(ctx: CanvasRenderingContext2D, game: Game): void {
...
function handleHighScoreInput(game: Game): void {
...
function renderHighScoreEntry(ctx: CanvasRenderingContext2D, game: Game): void {
  ctx.fillStyle = '#ffcc00';
  ...
  ctx.fillStyle = '#ffffff';
  ctx.font = '12px monospace';
  ctx.fillText(`SCORE: ${game.player?.score ?? 0}`, CANVAS_WIDTH / 2, 230);
...
export function handleHighScoreTouches(canvas: HTMLCanvasElement, game: Game): void {
  canvas.addEventListener('touchstart', (e: TouchEvent): void => {
    const rect = canvas.getBoundingClientRect();
    const touch = e.changedTouches[0] ?? e.touches[0];
    if (!touch) return;
    const tx = ((touch.clientX - rect.left) / rect.width) * CANVAS_WIDTH;
    const ty = ((touch.clientY - rect.top) / rect.height) * CANVAS_HEIGHT;
    ...
    // Pause button: top-right 40x30 zone (check before double-tap)
