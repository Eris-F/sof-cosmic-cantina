...
import { sfxMenuSelect, sfxPowerUp } from './audio.js';
...
export function updateShopUI(shop, dt) {
  if (shop.flashTimer > 0) {
    shop.flashTimer = Math.max(0, shop.flashTimer - dt);
  }
}

export function renderShopUI(ctx, shop, time) {
  ...
  for (let vi = 0; vi < visibleItems && vi + shop.scrollOffset < items.length; vi++) {
    ...
    if (category.id === 'skins') {
      ...
      if (item.rainbow) {
        ...
        drawCatShip(ctx, previewX, previewY, item);
      }
      ...
      if (item.id === 'none') {
        ...
        for (let j = 0; j < 8; j++) {
          ...
          const tx = previewX + Math.cos(angle) * (8 - j);
          const ty = previewY + Math.sin(angle) * (8 - j);
        ...
        for (let j = 0; j < 6; j++) {
          ...
          const tx = previewX + Math.cos(angle) * (7 - j);
          const ty = previewY + Math.sin(angle) * (7 - j);
          ctx.fillStyle = item.color;
      ...
      if (item.id === 'flowers') {
        ...
        for (let r = 0; r < 3; r++) {
          for (let c = 0; c < 5; c++) {
            ctx.fillStyle = (r + c) % 2 === 0 ? item.color1 : item.color2;
...
function canAffordItem(shop, item) {
  return shop.wallet.coins >= item.price;
}

export function handleShopTouch(shop, tx, ty, game) {
  const tabY = 62;
  const tabW = CANVAS_WIDTH / CATEGORIES.length;
  const listY = 92;
  const itemH = 72;
  ...
  if (ty >= listY && ty < listY + visibleItems * itemH) {
    ...
    if (idx >= 0 && idx < items.length) {
      if (idx === shop.itemIndex) {
        performAction(shop, category, items[idx], game);
      } else {
        shop.itemIndex = idx;
        sfxMenuSelect();
      }
...
function performAction(shop, category, item, game) {
  const owned = isOwned(shop.owned, category.id, item.id);
  const equipped = shop.equipped[category.id] === item.id;
  ...
  if (owned) {
    ...
    shop.flashTimer = 1.5;
    sfxPowerUp();
    if (category.id === 'skins') game.skin = item;
  } else {
...
export function handleShopKeyboard(shop, game, consumeKey) {
  ...
  if (consumeKey('Enter') || consumeKey('Space')) {
    const category = CATEGORIES[shop.categoryIndex];
    const items = SHOP_ITEMS[category.id];
    const item = items[shop.itemIndex];
