...
export function renderPowerups(ctx, powerups, time) {
...
export function checkPowerupPickup(powerups, playerX, playerY, playerW, playerH) {
  for (let i = powerups.length - 1; i >= 0; i--) {
    const p = powerups[i];
...
export function updateActiveEffects(effects, dt) {
...
export function applyPowerup(effects, type) {
  switch (type) {
...
export function renderActiveEffects(ctx, effects, canvasW) {
  let y = 28;
