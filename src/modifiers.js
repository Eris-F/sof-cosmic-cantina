...
const RANDOM_POOL = MODIFIERS.filter((m) => m.id !== 'normal');

export function pickModifier(wave) {
  if (wave <= 2) return MODIFIERS[0]; // normal for first 2 waves
  return RANDOM_POOL[Math.floor(Math.random() * RANDOM_POOL.length)];
}

export function renderModifierBanner(ctx, modifier, timer) {
...
export function renderModifierHUD(ctx, modifier) {
  if (!modifier || modifier.id === 'normal') return;
