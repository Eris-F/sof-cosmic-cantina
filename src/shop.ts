...
export type ShopCategoryId = 'skins' | 'bullets' | 'trails' | 'barriers';
...
export interface ItemStats {
  ...
  readonly startShield?: number | boolean;
  readonly startCompanion?: number | boolean;
...
export type ShopItem = SkinItem | BulletItem | TrailItem | BarrierItem;

export interface ShopItemsMap {
...
export const CATEGORIES: readonly ShopCategory[] = [
  { id: 'skins', name: 'CATS' },
...
export const SHOP_ITEMS: ShopItemsMap = {
  skins: [
    // --- CATS ---
    {
...
export function getEquippedStats(): CombinedStats {
  const equipped = loadEquipped();
  const combined: CombinedStats = {};

  for (const category of Object.keys(equipped) as ShopCategoryId[]) {
    ...
    for (const [key, val] of Object.entries(item.stats)) {
      if (typeof val === 'boolean') {
        ...
        if (key.endsWith('Mul') || key.endsWith('Chance')) {
          combined[key] = ((combined[key] as number) || 1) * val;
        } else {
          combined[key] = ((combined[key] as number) || 0) + val;
        }
      }
    }
  }
...
export function loadWallet(): Wallet {
  try {
    ...
    if (data) return JSON.parse(data) as Wallet;
  } catch (_) {}
...
export const COIN_REWARDS: CoinRewards = {
  kill: 4,
  eliteKill: 20,
  bossKill: 200,
  ufoKill: 40,
...
export function loadOwned(): OwnedItems {
  try {
    const data = localStorage.getItem('sofia_cantina_owned');
    if (data) return JSON.parse(data) as OwnedItems;
  } catch (_) {}
...
export function saveOwned(owned: OwnedItems): void {
  try {
    localStorage.setItem('sofia_cantina_owned', JSON.stringify(owned));
  } catch (_) {}
}

export function isOwned(owned: OwnedItems, category: string, itemId: string): boolean {
  const items = owned[category as keyof OwnedItems];
  return items !== undefined && items.includes(itemId);
}

export function buyItem(wallet: Wallet, owned: OwnedItems, category: string, itemId: string): BuyResult {
  const items = SHOP_ITEMS[category as ShopCategoryId];
  ...
  const newWallet: Wallet = { ...wallet, coins: wallet.coins - item.price };
  const categoryKey = category as keyof OwnedItems;
  const newOwned: OwnedItems = {
    ...owned,
    [categoryKey]: [...(owned[categoryKey] || []), itemId],
  };
...
export function loadEquipped(): EquippedItems {
  try {
    const data = localStorage.getItem('sofia_cantina_equipped');
    if (data) return JSON.parse(data) as EquippedItems;
  } catch (_) {}
  return {
    skins: 'default',
    bullets: 'bread',
...
export function saveEquipped(equipped: EquippedItems): void {
  try {
    localStorage.setItem('sofia_cantina_equipped', JSON.stringify(equipped));
  } catch (_) {}
}

export function equipItem(equipped: EquippedItems, category: string, itemId: string): EquippedItems {
  const newEquipped = { ...equipped, [category]: itemId } as EquippedItems;
  saveEquipped(newEquipped);
  return newEquipped;
}

export function getEquippedSkin(): SkinItem {
  const equipped = loadEquipped();
  const skins = SHOP_ITEMS.skins;
  return skins.find((s) => s.id === equipped.skins) ?? skins[0]!;
}

export function getEquippedBullet(): BulletItem {
  const equipped = loadEquipped();
  const bullets = SHOP_ITEMS.bullets;
  return bullets.find((b) => b.id === equipped.bullets) ?? bullets[0]!;
}

export function getEquippedTrail(): TrailItem {
  const equipped = loadEquipped();
  const trails = SHOP_ITEMS.trails;
  return trails.find((t) => t.id === equipped.trails) ?? trails[0]!;
}

export function getEquippedBarrier(): BarrierItem {
  const equipped = loadEquipped();
