...
interface TutorialPage {
  ...
  readonly draw: (ctx: CanvasRenderingContext2D, time: number) => void;
}
...
const PAGES: readonly TutorialPage[] = [
  ...
  },
  {
    ...
    draw: (ctx: CanvasRenderingContext2D, _time: number): void => {
      ...
      const weapons: readonly { readonly name: string; readonly desc: string; readonly color: string }[] = [
      ...
      for (let i = 0; i < weapons.length; i++) {
        ...
        ctx.fillStyle = '#666';
        ctx.font = '9px monospace';
        ctx.fillText(w.desc, CANVAS_WIDTH / 2 + 60, y);
      }
      ctx.textAlign = 'left';
    },
  },
  {
    ...
    draw: (ctx: CanvasRenderingContext2D, _time: number): void => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        ctx.font = 'bold 10px monospace';
        ctx.textAlign = 'center';
        ctx.fillText(item.letter, CANVAS_WIDTH / 2 - 82, y + 3);
        ctx.fillStyle = '#ccc';
        ctx.font = '9px monospace';
        ctx.textAlign = 'left';
        ctx.fillText(item.label, CANVAS_WIDTH / 2 - 65, y + 3);
      }
      ctx.textAlign = 'center';
      ctx.fillStyle = '#666';
      ctx.font = '9px monospace';
...
export function renderTutorial(ctx: CanvasRenderingContext2D, tutorial: TutorialState, time: number): void {
...
export function getTutorialPageCount(): number {
  return PAGES.length;
}
